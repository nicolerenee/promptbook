...
CREATE TABLE `new_recording_versions` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `file_path` text NOT NULL, `file_size_bytes` integer NOT NULL DEFAULT (0), `container` text NOT NULL DEFAULT (''), `quality` text NOT NULL DEFAULT (''), `video_codec` text NOT NULL DEFAULT (''), `audio_codec` text NOT NULL DEFAULT (''), `format_label` text NOT NULL DEFAULT (''), `notes` text NOT NULL DEFAULT (''), `media_info_json` text NOT NULL DEFAULT (''), `source_folder` text NOT NULL DEFAULT (''), `added_at` datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP), `last_seen_at` datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP), `recording_id` integer NOT NULL, CONSTRAINT `recording_versions_recordings_versions` FOREIGN KEY (`recording_id`) REFERENCES `recordings` (`recording_id`) ON DELETE CASCADE);
-- copy rows from old table "recording_versions" to new temporary table "new_recording_versions"
INSERT INTO `new_recording_versions` (`id`, `file_path`, `file_size_bytes`, `container`, `quality`, `video_codec`, `audio_codec`, `format_label`, `notes`, `media_info_json`, `added_at`, `last_seen_at`, `recording_id`) SELECT `id`, `file_path`, `file_size_bytes`, `container`, `quality`, `video_codec`, `audio_codec`, `format_label`, `notes`, `media_info_json`, `added_at`, `last_seen_at`, `recording_id` FROM `recording_versions`;
