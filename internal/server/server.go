...
type Options struct {
	DB     *sql.DB
	Logger zerolog.Logger
	// Stagemedia is the optional StageMedia.me image client. When nil, the
	// people detail page renders without headshots; the rest of the server
...
// Stagemedia returns the optional stagemedia client. nil when the server
// was constructed without one — handlers must guard before calling.
