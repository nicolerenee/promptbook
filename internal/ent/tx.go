...
func (tx *Tx) init() {
	...
	tx.ManualImportQueue = NewManualImportQueueClient(tx.config)
	tx.Performer = NewPerformerClient(tx.config)
	tx.Profile = NewProfileClient(tx.config)
	tx.Recording = NewRecordingClient(tx.config)
	tx.RecordingImageChoice = NewRecordingImageChoiceClient(tx.config)
	tx.RecordingVersion = NewRecordingVersionClient(tx.config)
	tx.Show = NewShowClient(tx.config)
	tx.SyncRun = NewSyncRunClient(tx.config)
	tx.WantsEntry = NewWantsEntryClient(tx.config)
}
...
// applies a query, for example: CastEntry.QueryXXX(), the query will be executed
